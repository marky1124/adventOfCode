...
class Solve
  ...
  def initialize
    ...
    @max_col = input.each_slice(2).map(&:first).map(&:to_i).max * 2
    @max_row = input.each_slice(2).map(&:last).map(&:to_i).max + 2
    ...
    # Add a floor
    generate_points_between("0,#{@max_row}", "#{@max_col},#{@max_row}").each do |col, row|
      @map[col_row_to_index(col, row)] = '#'
    end
