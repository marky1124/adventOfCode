...
class Puzzle
  ...
  def process_input(filename)
    ...
    lines.each_with_index do |line, lindex|
      line.chars.each_with_index do |char, cindex|
        @cubes["#{lindex},#{cindex},#{z}"] = char == '#'
      end
    end
  end

  def active_neighbours(pos)
    (x, y, z) = pos.split(',').map(&:to_i)
  ...
  # For the sample 3x3x1 starting state:
  # cycle 0 = 3x3x1 (x-y-z) where x=0..2, y=0..2, z=0
  # cycle 1 = 5x5x3 where x=-1..3, y=-1..3, z=-1..1
  # cycle 2 = 7x7x5 where x=-2..4, y=-2..4, z=-2..2
  # For the main input 8x8x1 starting state:
  # cycle 0 = 8x8x1 (x-y-z) where x=0..7, y=0..7, z=0
  # cycle 1 = 10x10x3 (x-y-z) where x=-1..8, y=-1..8, z=-1..1
